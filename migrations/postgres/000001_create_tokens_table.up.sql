...
CREATE INDEX IF NOT EXISTS idx_tokens_symbol_active ON tokens(symbol) WHERE is_active = true;
CREATE INDEX IF NOT EXISTS idx_tokens_rank ON tokens(market_cap_rank) WHERE market_cap_rank IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_tokens_metadata ON tokens USING GIN(metadata);
CREATE INDEX IF NOT EXISTS idx_tokens_last_update ON tokens(last_price_update);
CREATE INDEX IF NOT EXISTS idx_tokens_categories ON tokens USING GIN(categories);
